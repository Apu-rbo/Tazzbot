...
module.exports = {
    ...
    async execute(interaction) {
        ...
        // Ensure the target is a text-based channel
        ...
        try {
            ...
            if (targetUser) {
                // ── Filter by user ──
                // Fetch enough messages to find `amount` from the target user.
                // We fetch in batches of 100 (Discord max) until we have enough or run out.
                let collected = [];
                ...
                outerLoop: while (collected.length < amount) {
                    ...
                    for (const [, msg] of fetched) {
                        // Skip messages older than 14 days (can't bulk-delete them)
                ...
                // Bulk delete in one go (Discord requires 2+ for bulkDelete)
                ...
                // ── No user filter — delete the last `amount` messages ──
                const result = await targetCh.bulkDelete(amount, true); // true = skip old messages
                deleted = result.size;
            }
            ...
            return interaction.editReply(`❌ An error occurred while deleting messages: ${err.message}`);
        }
    },
};
